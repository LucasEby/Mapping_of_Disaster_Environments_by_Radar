...
class CubeListCreator:
    ...
    def plotCubes(self) -> None:
        ...
        if self.__all_real_points:
            for each_cube in self.__cube_list:
                # each_cube.z = each_cube.z * self.zoom
                each_cube.drawCube(each_cube.set_vertices())
        else:
            for index in range(self.__index_pos):
                # self.__cube_list[index].z = self.__cube_list[index].z * self.zoom
    ...
    def plotCubes(self, x_rotation, y_rotation, z_translation, y_translation) -> None:
        """
        This function is used to plot the cubes on the screen.
        :return: nothing
        """
        ...
        def save_file(self, filename) -> None:
            ...
            if self.__all_real_points:
                for each_cube in self.__cube_list:
                    ...
                    send_me.append(temp[0] + " " + temp[1] + " " + temp[2] + " " + temp[3] + " " + temp[4] + " "
                                   + temp[5] + " " + temp[6] + " " + "\n")
